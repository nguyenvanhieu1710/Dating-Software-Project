...
class NotificationService {
   ...
   */
  async getAllNotifications(params?: NotificationQueryParams): Promise<ApiResponse<PaginatedResponse<INotification>>> {
   ...
   */
  async getNotificationsByUserId(userId: number, params?: NotificationQueryParams): Promise<ApiResponse<PaginatedResponse<INotification>>> {
    const queryString = this.buildQueryString(params);
    return httpService.get<ApiResponse<PaginatedResponse<INotification>>>(
  ...
  async getNotificationById(id: number): Promise<ApiResponse<INotification>> {
    return httpService.get<ApiResponse<INotification>>(`${this.basePath}/${id}`);
  }

  /**
   * Search notifications by keyword
   */
  async searchNotifications(keyword: string): Promise<SearchNotificationsResponse> {
   ...
   */
  async createNotification(notificationData: CreateNotificationRequest): Promise<ApiResponse<INotification>> {
    return httpService.post<ApiResponse<INotification>>(this.basePath, notificationData);
  }

  /**
   * Send push notification
   */
  async sendPushNotification(notificationData: CreateNotificationRequest): Promise<ApiResponse<INotification>> {
    return httpService.post<ApiResponse<INotification>>(`${this.basePath}/push`, notificationData);
  }
  ...
  async markAllAsRead(userId: number): Promise<ApiResponse<void>> {
    return httpService.put<ApiResponse<void>>(`${this.basePath}/read-all`, { user_id: userId });
  }
  ...
  async deleteNotification(id: number): Promise<ApiResponse<INotification>> {
    return httpService.delete<ApiResponse<INotification>>(`${this.basePath}/${id}`);
  }
   ...
   */
  validateNotificationData(notificationData: CreateNotificationRequest): string[] {
    const errors: string[] = [];

    if (!notificationData.user_id || !Number.isInteger(notificationData.user_id) || notificationData.user_id < 1) {
    ...
    if (notificationData.is_read !== undefined && typeof notificationData.is_read !== 'boolean') {
      errors.push('is_read must be a boolean value');
    }
  ...
  validatePaginationParams(params: NotificationQueryParams): string[] {
    ...
    if (params.user_id !== undefined && (!Number.isInteger(params.user_id) || params.user_id < 1)) {
      errors.push('Valid user_id is required');
    }

    if (params.is_read !== undefined && typeof params.is_read !== 'boolean') {
      errors.push('is_read must be a boolean value');
    }
  ...
  formatNotificationForDisplay(notification: INotification): INotification & {
    ...
    const statusDisplay = notification.is_read ? 'Read' : 'Unread';
   ...
   */
  sortNotificationsBySentAt(notifications: INotification[], ascending: boolean = false): INotification[] {
   ...
   */
  groupNotificationsByType(notifications: INotification[]): Record<string, INotification[]> {
   ...
   */
  filterNotificationsByReadStatus(notifications: INotification[], isRead: boolean): INotification[] {
   ...
   */
  filterNotificationsByType(notifications: INotification[], type: string): INotification[] {
    return notifications.filter((notification) => notification.data?.type === type);
  }
  ...
  getUniqueTypes(notifications: INotification[]): string[] {
    const types = notifications.map((notification) => notification.data?.type || 'Unknown');
   ...
   */
  searchNotificationsLocally(notifications: INotification[], keyword: string): INotification[] {
    const lowerKeyword = keyword.toLowerCase();
    return notifications.filter(
      (notification) =>
        notification.title.toLowerCase().includes(lowerKeyword) ||
        notification.body.toLowerCase().includes(lowerKeyword) ||
        (notification.data?.type && notification.data.type.toLowerCase().includes(lowerKeyword))
    );
  }
